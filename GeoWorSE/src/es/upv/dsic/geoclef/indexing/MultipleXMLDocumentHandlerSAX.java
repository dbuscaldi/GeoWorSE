...
public class MultipleXMLDocumentHandlerSAX extends DefaultHandler {
  ...
  public MultipleXMLDocumentHandlerSAX(File xmlFile) 
 ...
 protected LocationsGroup geoEntities() {
	 ...
	 while(m.find()){
		 ...
		 try {
			...
			if(idxWord != null){
				int numSenses = idxWord.getWordIDs().size();
				/*if(idxWord.getWordIDs().size() > 1) {
					System.err.println("ambiguous word: "+item);
				}*/
				for(int sn=0;sn< numSenses; sn++){
					IWordID wordID=idxWord.getWordIDs().get(sn);
