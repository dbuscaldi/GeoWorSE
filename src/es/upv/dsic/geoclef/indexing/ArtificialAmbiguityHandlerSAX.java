...
public class ArtificialAmbiguityHandlerSAX extends DefaultHandler {
  ...
  protected String textBoundary;
  
  public ArtificialAmbiguityHandlerSAX(File xmlFile, double error_percentage, int max_errors, int current_errors) 
  	throws ParserConfigurationException, SAXException, IOException {
	
    this.dict=GeoWorSE.dict;
    this.classifier=GeoWorSE.classifier;
    this.gwDict=GeoWorSE.geoWNdict;
    if(!IndexArtificialAmbiguousToponyms.PASSAGE_INDEXING_ON) textBoundary="DOC";
    else textBoundary="PASSAGE";
    ...
    String eName = localName;
     ...
     if (attrs != null) {
       for (int i = 0; i < attrs.getLength(); i++) {
         ...
         // System.out.println("element "+eName+" : attr " + aName+"="+attrs.getValue(i));
         if(IndexArtificialAmbiguousToponyms.PASSAGE_INDEXING_ON){
        	 if(eName=="PASSAGE" && aName.equals("id")){
        		 docID.append(attrs.getValue(i));
        	 }
         }
       }
     }
  ...
  public void characters(char[] text, int start, int length)
	...
	if(!IndexArtificialAmbiguousToponyms.PASSAGE_INDEXING_ON){
  		...
  		if (stackel.passageTextReady() || stackel.headlineReady()) {
	  		textBuffer.append(text, start, length);
	  	}
  	}
	 ...
	 while(m.find()){
		 ...
		 try {
			...
			if(idxWord != null){
				...
				//do not process ambiguous toponyms
				if(idxWord.getWordIDs().size() == 1) {
					//System.err.println("unambiguous toponym: "+item);
					...
					boolean ambiguation=false;
					if(IndexArtificialAmbiguousToponyms.current_errors < IndexArtificialAmbiguousToponyms.max_errors){
						double rand = Math.random();
						//System.err.println("random value: "+rand+" current errors: "+current_errors+" err percentage:"+error_percentage);
					    
						if(rand <= IndexArtificialAmbiguousToponyms.error_percentage){
							//ambiguation process
							//System.err.println("ambiguating...");
							
							ISynset holo= this.getFirstHolonym(wsyn);
							//String holo_lemma = holo.getWord(1).getLemma();
							List<ISynsetID> sisterTerms = this.getSisterTerms(holo);
							int stSize=sisterTerms.size();
							Random rgen = new Random();
							ISynset new_holo = dict.getSynset(sisterTerms.get(rgen.nextInt(stSize)));
							//String new_holo_lemma = new_holo.getWord(1).getLemma();
							List<IWord> holo_ws = new_holo.getWords();
							for(IWord iw : holo_ws){
								locs.addSCelem(iw.getLemma().replace('_', ' '));
							}
							Vector<String> new_holos= this.getInheritedHolonyms(new_holo);
							for(int j=0; j < new_holos.size(); j++){
								locs.addSCelem(new_holos.elementAt(j));
							}
							
							//System.err.println(">>> ambiguation: "+word.getLemma()+" from "+holo_lemma+" to "+new_holo_lemma);
							//System.in.read();
							ambiguation = true;
							IndexArtificialAmbiguousToponyms.current_errors++;
						}
					}
 ...
 protected ISynset getFirstHolonym(ISynset syn){
	 List<ISynsetID> holos = syn.getRelatedSynsets(Pointer.HOLONYM_PART);
	 ISynsetID id=holos.get(0);
	 ISynset holonym = dict.getSynset(id);
	 return holonym;
 }
 
 protected List<ISynsetID> getSisterTerms(ISynset syn){
	 //meronyms of the holonym without syn
	 List<ISynsetID> holos = syn.getRelatedSynsets(Pointer.HOLONYM_PART);
	 ISynsetID id_holo=holos.get(0);
	 ISynset holonym = dict.getSynset(id_holo);
	 List<ISynsetID> meros = holonym.getRelatedSynsets(Pointer.MERONYM_PART);
	 LinkedList<ISynsetID> meros_tmp = new LinkedList<ISynsetID>(meros);
	 ISynsetID id_syn=syn.getID();
	 meros_tmp.remove(id_syn);
	 meros=meros_tmp.subList(0, meros_tmp.size());
	 return meros;
